...
function [tv, ym] = RungeKuttaI_Anderson1(fun, t0, y0, metodo, Deltat, tf)
    ...
    while tn<tf
        ...
        %Matriz jacobiana R y factorizaci'on (suponemos matriz llena):
        Rm          = eye(length(yn)) - Deltatn*A(s,s)*df_dy_n;
        [L,U,P]     = lu(Rm);
        ...
        for ii=2:s
            ...
            aux_ii              = yn + Deltatn*S;
            %Resolver:
            [y_ii, ~, flag]     = Anderson1(@gfun, yn, 1e-10, 100, 10);
            if flag<0
    ...
    function g = gfun(y)
        ...
        %Residuo precondicionado (suponemos matriz llena):
        g           = U\(L\(P*r));
