function [f,J] = Burgers1D_DF_Residuo2(xv, epsilon, uv, CalcJ)
    ...
    for ii=2:Nx-1
        ...
        f(ii)   = uv(ii) * (uv(ii) - uv(ii-1))/hx - ...
                    epsilon*( uv(ii+1)-2*uv(ii)+uv(ii-1) )/hx^2;
        if CalcJ

            %Derivadas de f(I) respecto de...:
            %Nodo central (ii):
            dfI_du_C    = (2*uv(ii)-uv(ii-1))/hx + 2*epsilon/hx^2;
            %Nodo izquierdo (ii-1)
            dfI_du_L    = -uv(ii)/hx - epsilon/hx^2;
            %Nodo derecho (ii+1):
            dfI_du_R    = - epsilon/hx^2;
