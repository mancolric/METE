function [f,J] = Burgers1D_DF_conservativo_Residuo1(xv, epsilon, uv, CalcJ)
    ...
    for ii=2:Nx-1
        ...
        %Imponemos
        %   (1/2*u_(i+1)^2 - 1/2*u_(i-1)^2)/(2*hx) - ...
        %       epsilon*( u_(i+1)-2*u_(i)+u_(i-1) )/hx^2 = 0
        f(ii)   = (0.5*uv(ii+1)^2 - 0.5*uv(ii-1)^2)/(2*hx) - ...
                    epsilon*( uv(ii+1)-2*uv(ii)+uv(ii-1) )/hx^2;
        if CalcJ

            %Derivadas de f(I) respecto de...:
            %Nodo central (ii):
            dfI_du_C    = 2*epsilon/hx^2;
            %Nodo izquierdo (ii-1)
            dfI_du_L    = -uv(ii-1)/(2*hx) - epsilon/hx^2;
            %Nodo derecho (ii+1):
            dfI_du_R    = +uv(ii+1)/(2*hx) - epsilon/hx^2;
